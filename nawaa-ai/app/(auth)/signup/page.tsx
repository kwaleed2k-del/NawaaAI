...
const patternSvg = (
  <svg className="absolute inset-0 h-full w-full opacity-[0.06]" viewBox="0 0 100 100" preserveAspectRatio="none">
    <defs>
      <pattern id="islamic-signup" width="20" height="20" patternUnits="userSpaceOnUse">
        <path d="M10 0 L20 10 L10 20 L0 10 Z" fill="currentColor" className="text-[#C9A84C]" />
      </pattern>
    </defs>
    <rect width="100%" height="100%" fill="url(#islamic-signup)" />
  </svg>
);
    ...
    <div className="flex min-h-screen" dir={isRtl ? "rtl" : "ltr"}>
      <div className="relative hidden w-1/2 flex-col justify-center bg-[#020B05] lg:flex">
        ...
        <div className="relative z-10 px-12">
          ...
          <p className="mt-4 max-w-sm text-[#7B9E86]">{messages[locale].auth.joinNawaa}</p>
        </div>
      </div>
      <div className="flex flex-1 flex-col justify-center bg-[#0B1A0F] px-6 py-12 lg:px-16">
        ...
        >
          <h1 className="text-2xl font-bold text-[#D0EBDA]">{t.joinNawaa}</h1>
          <p className="mt-1 text-sm text-[#7B9E86]">{t.createAccount}</p>
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            <div>
              <Label className="text-[#D0EBDA]">{t.fullName}</Label>
              <Input
                value={fullName}
            ...
            <div>
              <Label className="text-[#D0EBDA]">{t.confirmPassword}</Label>
              <Input
                type="password"
                value={confirm}
            ...
            <label className="flex items-center gap-2 text-sm text-[#7B9E86]">
              <input
                type="checkbox"
                checked={terms}
                onChange={(e) => setTerms(e.target.checked)}
                className="rounded border-[#172E1F] bg-[#020B05]"
              />
              {t.agreeTerms}
            </label>
            <Button
          ...
          <p className="mt-6 text-center text-sm text-[#7B9E86]">
            {t.hasAccount}{" "}
            <Link href="/login" className="text-[#00A352] hover:underline">
              {t.signIn}
            </Link>
          </p>
        </motion.div>
