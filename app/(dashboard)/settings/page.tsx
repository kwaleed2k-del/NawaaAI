  ...
  return (
    <div dir={isRtl ? "rtl" : "ltr"} className="space-y-10 pb-16">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20">
              <Settings className="h-6 w-6 text-emerald-200" />
            </div>
            <span className="text-lg font-bold text-emerald-200/80 tracking-wide">{locale === "ar" ? "\u0627\u0644\u0625\u0639\u062f\u0627\u062f\u0627\u062a" : "Settings"}</span>
          </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {/* ── Profile Section ── */}
        <div className="group relative overflow-hidden rounded-2xl border-2 border-[#D4EBD9] bg-white shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div className="p-8">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-[#004D26]">{t.profile}</h2>
            </div>
            <div className="space-y-6">
              <div>
                <label className="block text-lg font-bold text-[#004D26] mb-2">{t.fullName}</label>
                <input
                  type="text"
                  value={fullName}
                  onChange={(e) => setFullName(e.target.value)}
                  className="h-14 w-full rounded-2xl border-2 border-[#D4EBD9] bg-[#F8FBF8] px-5 text-lg text-[#0A1F0F] focus:outline-none focus:border-[#006C35] focus:shadow-[0_0_0_3px_rgba(0,108,53,0.08)] transition-all"
                />
              </div>
              <div>
                <label className="block text-lg font-bold text-[#004D26] mb-2">{t.email}</label>
                <input
                  type="email"
                  value={user?.email || ""}
                  readOnly
                  className="h-14 w-full rounded-2xl border-2 border-[#D4EBD9] bg-[#F0F7F2] px-5 text-lg text-[#5A8A6A] cursor-not-allowed"
                />
        ...
        <div className="group relative overflow-hidden rounded-2xl border-2 border-[#D4EBD9] bg-white shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div className="p-8">
            <div className="flex items-center gap-4 mb-8">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 shadow-lg shadow-amber-400/20">
                <Building2 className="h-7 w-7 text-white" />
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-[#004D26]">{t.agency}</h2>
            </div>
            <div className="space-y-6">
              <div>
                <label className="block text-lg font-bold text-[#004D26] mb-2">{t.agencyName}</label>
                <input
                  type="text"
                  value={agencyName}
                  onChange={(e) => setAgencyName(e.target.value)}
                  placeholder={t.agencyNamePlaceholder}
                  className="h-14 w-full rounded-2xl border-2 border-[#D4EBD9] bg-[#F8FBF8] px-5 text-lg text-[#0A1F0F] placeholder:text-[#5A8A6A]/50 focus:outline-none focus:border-[#006C35] focus:shadow-[0_0_0_3px_rgba(0,108,53,0.08)] transition-all"
                />
              </div>
              <div>
                <label className="block text-lg font-bold text-[#004D26] mb-2">{t.agencyType}</label>
                <select
                  value={agencyType}
        ...
        <div className="lg:col-span-2 group relative overflow-hidden rounded-2xl border-2 border-[#D4EBD9] bg-white shadow-lg hover:shadow-xl transition-all duration-300">
          <div className="absolute top-0 inset-x-0 h-1.5 bg-gradient-to-r from-violet-500 to-purple-600" />
          <div className="p-8">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-[#004D26]">{t.preferences}</h2>
            </div>
            <div>
              <label className="block text-lg font-bold text-[#004D26] mb-4">{t.language}</label>
              <div className="grid grid-cols-2 gap-4 max-w-lg">
                <button
                  ...
                  className={`relative h-16 rounded-2xl border-2 text-xl font-bold transition-all duration-300 ${
                    locale === "en"
                      ? "border-[#006C35] bg-gradient-to-r from-[#006C35] to-[#00A352] text-white shadow-lg shadow-emerald-500/20"
                      : "border-[#D4EBD9] bg-white text-[#5A8A6A] hover:border-[#006C35]/40 hover:shadow-md"
                  }`}
                >
      ...
      <div className="flex flex-col items-center gap-4">
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="relative w-full sm:w-auto sm:min-w-[320px] h-16 rounded-2xl bg-gradient-to-r from-[#006C35] to-[#00A352] text-xl font-black text-white shadow-xl shadow-emerald-500/20 hover:shadow-2xl hover:shadow-emerald-500/30 hover:scale-[1.02] transition-all duration-300 disabled:opacity-60 disabled:hover:scale-100 overflow-hidden"
        >
        ...
        {saved && (
          <div className="flex items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 px-6 py-3 shadow-sm">
            <Check className="h-5 w-5 text-emerald-600" />
            <p className="text-lg font-bold text-emerald-700">{t.saved}</p>
          </div>
        )}
      </div>
    </div>
