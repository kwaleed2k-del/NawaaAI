  ...
  return (
    <div className="flex items-center gap-3">
      <div className={`flex-1 ${height} bg-[#E8F0EA] rounded-full overflow-hidden`}>
        <div className="h-full rounded-full transition-all duration-700 ease-out" style={{ width: `${score}%`, backgroundColor: color || (score >= 70 ? "#006C35" : score >= 40 ? "#F59E0B" : "#ef4444") }} />
      </div>
...
function ScoreCircle({ score, size = 80, label }: { score: number; size?: number; label?: string }) {
  ...
  const color = score >= 70 ? "#006C35" : score >= 40 ? "#F59E0B" : "#ef4444";
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      {label && <span className="text-lg font-bold text-[#5A8A6A] text-center">{label}</span>}
    </div>
  );
}
...
function SectionTitle({ icon: Icon, children, color = "text-[#006C35]" }: { icon: React.ElementType; children: React.ReactNode; color?: string }) {
  return <h3 className={`flex items-center gap-3 text-2xl font-extrabold text-[#0A1F0F] mb-5`}><Icon className={`h-6 w-6 ${color}`} />{children}</h3>;
}
  ...
  return (
    <div className="space-y-8 w-full" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="flex items-center justify-end gap-3 flex-wrap">
        {analysisData && (
          <>
            <button onClick={handleExportPdf} className="flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-[#D4EBD9] text-lg font-bold text-[#2D5A3D] hover:bg-[#F0F7F2] transition-colors">
              <Download className="h-5 w-5" /> {t.exportPdf}
            </button>
            <button onClick={saveAnalysis} disabled={saving} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-[#006C35] to-[#00A352] text-lg font-bold text-white shadow-md hover:shadow-lg transition-all">
              {saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}
              {saveStatus || t.save}
            </button>
          </>
        )}
        <button onClick={() => setShowPrevious(!showPrevious)} className="flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-[#D4EBD9] text-lg font-bold text-[#2D5A3D] hover:bg-[#F0F7F2] transition-colors">
      ...
      {showPrevious && (
        <SectionCard>
          <h3 className="font-extrabold text-2xl text-[#0A1F0F] mb-4">{t.loadPrevious}</h3>
          {savedAnalyses.length === 0 ? <p className="text-lg text-[#5A8A6A]">{t.noPreviousAnalyses}</p> : (
            <div className="space-y-3">{savedAnalyses.map((sa) => (
              <div key={sa.id} className="flex items-center justify-between p-4 rounded-xl border-2 border-[#D4EBD9] hover:bg-[#F0F7F2] transition-colors">
                <div><p className="text-lg font-bold text-[#0A1F0F]">{sa.competitors?.map((c) => c.name).join(", ")}</p><p className="text-sm text-[#5A8A6A]">{new Date(sa.created_at).toLocaleDateString()}</p></div>
                <div className="flex gap-2">
                  <button onClick={() => loadAnalysis(sa)} className="px-4 py-2 rounded-xl border-2 border-[#006C35] text-lg font-bold text-[#006C35] hover:bg-[#F0F7F2]">{t.load}</button>
      ...
      <SectionCard>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-extrabold text-2xl text-[#0A1F0F]">{locale === "ar" ? "اختر الشركة" : "Select Company"}</h3>
        </div>
        {companies.length === 0 ? (
          <p className="text-lg text-[#5A8A6A]">{locale === "ar" ? "لا توجد شركات. أضف شركة أولاً." : "No companies found. Add one from Companies page."}</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {companies.map((c) => (
              <button key={c.id} onClick={() => setSelectedCompany(c)} className={`p-5 rounded-2xl border-2 text-start transition-all ${selectedCompany?.id === c.id ? "border-[#006C35] bg-[#F0F7F2] shadow-[0_0_0_4px_rgba(0,108,53,0.1)]" : "border-[#D4EBD9] hover:border-[#00A352] hover:bg-[#F8FBF8]"}`}>
                <p className="font-bold text-xl text-[#0A1F0F] truncate">{c.name}</p>
                {c.industry && <p className="text-lg text-[#5A8A6A] mt-1">{c.industry}</p>}
              </button>
            ))}
          </div>
        )}
      ...
      <SectionCard className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-extrabold text-2xl text-[#0A1F0F]">{t.addCompetitor}</h3>
            <p className="text-lg text-[#5A8A6A]">{t.maxCompetitors} · {competitors.length}/5</p>
          </div>
          <button onClick={addCompetitor} disabled={competitors.length >= 5} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-[#D4EBD9] text-lg font-bold text-[#006C35] hover:bg-[#F0F7F2] disabled:opacity-40 transition-colors">
        ...
        {competitors.map((comp, idx) => (
          <div key={idx} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_160px_1fr_auto] gap-4 items-end p-5 rounded-2xl border-2 border-[#D4EBD9] bg-[#F8FBF8]">
            <div>
              <label className="text-lg font-bold text-[#5A8A6A] mb-1.5 block">{t.competitorName}</label>
              <input value={comp.name} onChange={(e) => updateCompetitor(idx, "name", e.target.value)} placeholder={locale === "ar" ? "اسم المنافس" : "Competitor name"} className="w-full h-12 rounded-xl border-2 border-[#D4EBD9] bg-white px-4 text-lg text-[#0A1F0F] focus:border-[#006C35] focus:outline-none transition-colors" />
            </div>
            <div>
              <label className="text-lg font-bold text-[#5A8A6A] mb-1.5 block">{t.socialHandle}</label>
              <input value={comp.handle} onChange={(e) => updateCompetitor(idx, "handle", e.target.value)} placeholder="@handle" className="w-full h-12 rounded-xl border-2 border-[#D4EBD9] bg-white px-4 text-lg text-[#0A1F0F] focus:border-[#006C35] focus:outline-none transition-colors" />
            </div>
            <div>
              <label className="text-lg font-bold text-[#5A8A6A] mb-1.5 block">{t.platform}</label>
              <select value={comp.platform} onChange={(e) => updateCompetitor(idx, "platform", e.target.value)} className="w-full h-12 rounded-xl border-2 border-[#D4EBD9] bg-white px-4 text-lg text-[#0A1F0F] focus:border-[#006C35] focus:outline-none transition-colors">
                {PLATFORMS.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-lg font-bold text-[#5A8A6A] mb-1.5 block">{t.websiteUrl}</label>
              <input value={comp.websiteUrl} onChange={(e) => updateCompetitor(idx, "websiteUrl", e.target.value)} placeholder="https://..." className="w-full h-12 rounded-xl border-2 border-[#D4EBD9] bg-white px-4 text-lg text-[#0A1F0F] focus:border-[#006C35] focus:outline-none transition-colors" />
            </div>
        ...
        <div className="flex items-center justify-between p-5 rounded-2xl border-2 border-[#D4EBD9] bg-[#F8FBF8]">
          <div>
            <h4 className="font-bold text-lg text-[#0A1F0F]">{locale === "ar" ? "لغة التحليل" : "Analysis Language"}</h4>
            <p className="text-base text-[#5A8A6A]">{locale === "ar" ? "اختر لغة نتائج التحليل والتقرير" : "Choose the language for the analysis output & PDF report"}</p>
          </div>
          <div className="flex rounded-xl border-2 border-[#D4EBD9] overflow-hidden">
            <button
              onClick={() => setOutputLanguage("en")}
              className={`px-6 py-3 text-lg font-bold transition-all ${outputLanguage === "en" ? "bg-gradient-to-r from-[#006C35] to-[#00A352] text-white" : "bg-white text-[#5A8A6A] hover:bg-[#F0F7F2]"}`}
            >
              English
            </button>
            <button
              onClick={() => setOutputLanguage("ar")}
              className={`px-6 py-3 text-lg font-bold transition-all ${outputLanguage === "ar" ? "bg-gradient-to-r from-[#006C35] to-[#00A352] text-white" : "bg-white text-[#5A8A6A] hover:bg-[#F0F7F2]"}`}
            >
              العربية
            </button>
          </div>
        ...
        <button onClick={runAnalysis} disabled={loading} className="w-full h-14 rounded-2xl bg-gradient-to-r from-[#006C35] to-[#00A352] text-white text-lg font-bold shadow-[0_6px_24px_rgba(0,108,53,0.3)] hover:shadow-[0_8px_32px_rgba(0,108,53,0.4)] disabled:opacity-60 transition-all flex items-center justify-center gap-3">
        ...
        <div className="space-y-6">
          ...
          <div className="flex rounded-2xl border-2 border-[#D4EBD9] bg-white p-1.5 gap-1.5">
            {([
              ...
              <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex-1 flex items-center justify-center gap-2.5 py-4 rounded-xl text-lg font-bold transition-all ${activeTab === tab.key ? "bg-gradient-to-r from-[#006C35] to-[#00A352] text-white shadow-lg" : "text-[#5A8A6A] hover:bg-[#F0F7F2]"}`}>
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              ...
              <SectionCard>
                ...
                <p className="text-lg leading-relaxed text-[#2D5A3D] whitespace-pre-line">{analysisData.executiveSummary}</p>
              </SectionCard>
              ...
              <SectionCard>
                ...
                {analysisData.brandAssessment.marketPosition && (
                  <div className="p-5 rounded-2xl bg-gradient-to-r from-[#006C35]/5 to-[#00A352]/5 border border-[#D4EBD9] mb-6">
                    <p className="text-lg text-[#2D5A3D] leading-relaxed font-medium">{analysisData.brandAssessment.marketPosition}</p>
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-5 rounded-2xl bg-green-50/50 border border-green-200">
                    <h4 className="font-bold text-xl text-[#006C35] mb-3 flex items-center gap-2"><TrendingUp className="h-5 w-5" /> {t.strengths}</h4>
                    <ul className="space-y-2.5">{analysisData.brandAssessment.strengths.map((s, i) => <li key={i} className="text-lg text-[#2D5A3D] leading-relaxed flex items-start gap-2"><span className="text-[#006C35] font-bold text-xl mt-0.5">+</span><span>{s}</span></li>)}</ul>
                  </div>
                  <div className="p-5 rounded-2xl bg-red-50/50 border border-red-200">
                    ...
                    <ul className="space-y-2.5">{analysisData.brandAssessment.weaknesses.map((w, i) => <li key={i} className="text-lg text-[#2D5A3D] leading-relaxed flex items-start gap-2"><span className="text-red-500 font-bold text-xl mt-0.5">-</span><span>{w}</span></li>)}</ul>
                  </div>
                  {analysisData.brandAssessment.opportunities && (
                    <div className="p-5 rounded-2xl bg-blue-50/50 border border-blue-200">
                      <h4 className="font-bold text-xl text-blue-600 mb-3 flex items-center gap-2"><Sparkles className="h-5 w-5" /> {locale === "ar" ? "الفرص" : "Opportunities"}</h4>
                      <ul className="space-y-2.5">{analysisData.brandAssessment.opportunities.map((o, i) => <li key={i} className="text-lg text-[#2D5A3D] leading-relaxed flex items-start gap-2"><span className="text-blue-500 font-bold text-xl mt-0.5">★</span><span>{o}</span></li>)}</ul>
                    </div>
                  )}
                  {analysisData.brandAssessment.threats && (
                    <div className="p-5 rounded-2xl bg-orange-50/50 border border-orange-200">
                      ...
                      <ul className="space-y-2.5">{analysisData.brandAssessment.threats.map((t2, i) => <li key={i} className="text-lg text-[#2D5A3D] leading-relaxed flex items-start gap-2"><span className="text-orange-500 font-bold text-xl mt-0.5">!</span><span>{t2}</span></li>)}</ul>
                    </div>
                  )}
                </div>
              </SectionCard>
              ...
              {/* Competitor Cards */}
              <div>
                <h3 className="flex items-center gap-3 text-2xl font-extrabold text-[#0A1F0F] mb-5"><Swords className="h-6 w-6 text-[#006C35]" /> {t.competitorProfiles}</h3>
                <div className="space-y-6">
                    ...
                    <SectionCard key={i}>
                      <div className="flex items-start justify-between mb-5 flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="font-bold text-xl text-[#0A1F0F]">{comp.name}</h4>
                            <p className="text-lg text-[#5A8A6A] font-medium">{comp.handle} · {comp.platform}</p>
                          </div>
                        </div>
                      ...
                      {/* Key Insight Banner */}
                      <div className="p-5 rounded-2xl bg-gradient-to-r from-[#006C35]/5 to-[#7C3AED]/5 border-2 border-[#D4EBD9] mb-6">
                        <p className="text-lg font-bold text-[#006C35] uppercase tracking-wider mb-1">{locale === "ar" ? "الرؤية الرئيسية" : "KEY INSIGHT"}</p>
                        <p className="text-lg text-[#0A1F0F] leading-7 font-medium">{comp.keyInsight}</p>
                      </div>

                      {comp.stealThisMove && (
                        <div className="p-5 rounded-2xl bg-[#7C3AED]/5 border-2 border-[#7C3AED]/20 mb-6">
                          <p className="text-lg font-bold text-[#7C3AED] uppercase tracking-wider mb-1 flex items-center gap-2"><Star className="h-4 w-4" /> {locale === "ar" ? "اسرق هذه الحركة" : "STEAL THIS MOVE"}</p>
                          <p className="text-lg text-[#0A1F0F] leading-7 font-medium">{comp.stealThisMove}</p>
                        </div>
                      )}
                      ...
                      {(comp.companyOverview || comp.productsAndServices || comp.websiteAnalysis) && (
                        <div className="mb-6">
                          <h5 className="font-black text-xl text-[#0A1F0F] mb-4 flex items-center gap-2">
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {comp.companyOverview && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Building2 className="h-4 w-4" />{locale === "ar" ? "نظرة عامة على الشركة" : "Company Overview"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.companyOverview}</p>
                              </div>
                            )}
                            {comp.productsAndServices && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Package className="h-4 w-4" />{locale === "ar" ? "المنتجات والخدمات" : "Products & Services"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.productsAndServices}</p>
                              </div>
                            )}
                            {comp.targetMarket && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Users className="h-4 w-4" />{locale === "ar" ? "السوق المستهدف" : "Target Market"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.targetMarket}</p>
                              </div>
                            )}
                            {comp.brandPositioning && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Crosshair className="h-4 w-4" />{locale === "ar" ? "تموضع العلامة التجارية" : "Brand Positioning"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.brandPositioning}</p>
                              </div>
                            )}
                            {comp.websiteAnalysis && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Globe className="h-4 w-4" />{locale === "ar" ? "تحليل الموقع الإلكتروني" : "Website Analysis"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.websiteAnalysis}</p>
                              </div>
                            )}
                            {comp.digitalPresence && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Eye className="h-4 w-4" />{locale === "ar" ? "الحضور الرقمي" : "Digital Presence"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.digitalPresence}</p>
                              </div>
                            )}
                            {comp.pricingStrategy && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><DollarSign className="h-4 w-4" />{locale === "ar" ? "استراتيجية التسعير" : "Pricing Strategy"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.pricingStrategy}</p>
                              </div>
                            )}
                            {comp.customerReviews && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Star className="h-4 w-4" />{locale === "ar" ? "مراجعات العملاء" : "Customer Reviews"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.customerReviews}</p>
                              </div>
                            )}
                            {comp.technologyStack && (
                              <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                                <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><Layers className="h-4 w-4" />{locale === "ar" ? "البنية التقنية" : "Technology Stack"}</p>
                                <p className="text-lg text-[#2D5A3D] leading-7">{comp.technologyStack}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        <div className="space-y-4">
                          <h5 className="font-black text-xl text-[#0A1F0F] flex items-center gap-2">
                          ...
                          {[
                            ...
                            <div key={item.label} className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                              <p className="text-lg font-bold text-[#006C35] mb-1 flex items-center gap-1.5"><item.icon className="h-4 w-4" />{item.label}</p>
                              <p className="text-lg text-[#2D5A3D] leading-7">{item.value}</p>
                            </div>
                          ))}
                          {comp.captionStyle && (
                            <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                              <p className="text-lg font-bold text-[#006C35] mb-1">{locale === "ar" ? "أسلوب الكتابة" : "Caption Style"}</p>
                              <p className="text-lg text-[#2D5A3D] leading-7">{comp.captionStyle}</p>
                            </div>
                          )}
                        </div>
                        <div className="space-y-4">
                          {comp.audienceProfile && (
                            <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                              <p className="text-lg font-bold text-[#006C35] mb-1">{locale === "ar" ? "ملف الجمهور" : "Audience Profile"}</p>
                              <p className="text-lg text-[#2D5A3D] leading-7">{comp.audienceProfile}</p>
                            </div>
                          )}
                          {comp.contentCalendar && (
                            <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                              <p className="text-lg font-bold text-[#006C35] mb-1">{locale === "ar" ? "تقويم المحتوى" : "Content Calendar"}</p>
                              <p className="text-lg text-[#2D5A3D] leading-7">{comp.contentCalendar}</p>
                            </div>
                          )}
                          {comp.paidStrategy && (
                            <div className="p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                              <p className="text-lg font-bold text-[#006C35] mb-1">{locale === "ar" ? "الاستراتيجية المدفوعة" : "Paid Strategy"}</p>
                              <p className="text-lg text-[#2D5A3D] leading-7">{comp.paidStrategy}</p>
                            </div>
                          )}
                          <div className="p-5 rounded-xl bg-green-50/50 border border-green-200">
                            <p className="text-lg font-bold text-[#006C35] mb-2">{t.strengths}</p>
                            {comp.strengths.map((s, j) => <p key={j} className="text-lg text-[#2D5A3D] leading-7">+ {s}</p>)}
                          </div>
                          <div className="p-5 rounded-xl bg-red-50/50 border border-red-200">
                            ...
                            {comp.weakPoints.map((w, j) => <p key={j} className="text-lg text-[#2D5A3D] leading-7">- {w}</p>)}
                          </div>
                        </div>
                      </div>
                    </SectionCard>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    <div key={item.key} className="p-5 rounded-2xl bg-[#F8FBF8] border border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#006C35] mb-2 flex items-center gap-2"><item.icon className="h-5 w-5" />{item.label}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.saudiMarketInsights[item.key]}</p>
                    </div>
                  ))}
                  {analysisData.saudiMarketInsights.localOpportunities && (
                    <div className="p-5 rounded-2xl bg-[#F8FBF8] border border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#006C35] mb-2 flex items-center gap-2"><Sparkles className="h-5 w-5" />{locale === "ar" ? "فرص محلية" : "Local Opportunities"}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.saudiMarketInsights.localOpportunities}</p>
                    </div>
                  )}
                  {analysisData.saudiMarketInsights.ramadanStrategy && (
                    <div className="md:col-span-2 p-5 rounded-2xl bg-gradient-to-r from-[#006C35]/5 to-[#7C3AED]/5 border-2 border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#7C3AED] mb-2 flex items-center gap-2"><Star className="h-5 w-5" />{locale === "ar" ? "استراتيجية رمضان" : "Ramadan Strategy"}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.saudiMarketInsights.ramadanStrategy}</p>
                    </div>
                  )}
                </div>
              </SectionCard>
              ...
              {analysisData.industryAnalysis && (
                <SectionCard>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-5 rounded-2xl bg-[#F8FBF8] border border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#006C35] mb-2 flex items-center gap-2"><BarChart3 className="h-5 w-5" />{locale === "ar" ? "نظرة عامة على السوق" : "Market Overview"}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.industryAnalysis.marketOverview}</p>
                    </div>
                    <div className="p-5 rounded-2xl bg-[#F8FBF8] border border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#006C35] mb-2 flex items-center gap-2"><Swords className="h-5 w-5" />{locale === "ar" ? "المشهد التنافسي" : "Competitive Landscape"}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.industryAnalysis.competitiveLandscape}</p>
                    </div>
                    <div className="p-5 rounded-2xl bg-[#F8FBF8] border border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#006C35] mb-2 flex items-center gap-2"><Users className="h-5 w-5" />{locale === "ar" ? "اتجاهات المستهلكين" : "Consumer Trends"}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.industryAnalysis.consumerTrends}</p>
                    </div>
                    <div className="p-5 rounded-2xl bg-[#F8FBF8] border border-[#D4EBD9]">
                      <h4 className="font-black text-lg text-[#006C35] mb-2 flex items-center gap-2"><Rocket className="h-5 w-5" />{locale === "ar" ? "التوقعات المستقبلية" : "Future Outlook"}</h4>
                      <p className="text-lg text-[#2D5A3D] leading-7">{analysisData.industryAnalysis.futureOutlook}</p>
                    </div>
                  </div>
                </SectionCard>
              )}
            ...
            <div className="space-y-6">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-2 border-[#D4EBD9]">
                        <th className="text-start py-4 px-3 font-black text-lg text-[#5A8A6A]">{t.category}</th>
                        <th className="text-start py-4 px-3 font-black text-lg text-[#006C35]">{t.yourBrand}</th>
                        {Object.keys(analysisData.comparisonMatrix.competitors).map((n) => <th key={n} className="text-start py-4 px-3 font-black text-lg text-[#0A1F0F]">{n}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {analysisData.comparisonMatrix.categories.map((cat, idx) => (
                        <tr key={cat} className="border-b border-[#D4EBD9]/50 hover:bg-[#F8FBF8] transition-colors">
                          <td className="py-4 px-3 font-bold text-lg text-[#5A8A6A]">{cat}</td>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
                <div className="rounded-2xl border-2 border-[#006C35] bg-[#F0F7F2] p-6 flex flex-col items-center">
                  ...
                  <p className="text-lg font-black text-[#006C35] mt-2">{t.yourBrand}</p>
                </div>
                {analysisData.competitors.map((comp) => (
                  <div key={comp.name} className="rounded-2xl border-2 border-[#D4EBD9] bg-white p-6 flex flex-col items-center">
                    ...
                    <p className="text-lg font-black text-[#0A1F0F] mt-2">{comp.name}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              {analysisData.winningStrategy.quickWins && analysisData.winningStrategy.quickWins.length > 0 && (
                <SectionCard className="bg-gradient-to-r from-[#006C35]/5 to-[#00A352]/5 border-[#006C35]/20">
                  <SectionTitle icon={Zap} color="text-[#7C3AED]">{locale === "ar" ? "مكاسب سريعة — نفذها اليوم!" : "Quick Wins — Do These TODAY!"}</SectionTitle>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {analysisData.winningStrategy.quickWins.map((w, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-white border-2 border-[#D4EBD9]">
                        <div className="w-8 h-8 rounded-lg bg-[#7C3AED] flex items-center justify-center shrink-0"><span className="text-white font-black text-sm">{i + 1}</span></div>
                        <p className="text-lg text-[#0A1F0F] leading-7">{w}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
                  ...
                  {analysisData.winningStrategy.immediate.map((a, i) => (
                    <div key={i} className="p-5 rounded-2xl bg-red-50/50 border-2 border-red-100">
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <p className="text-lg text-[#0A1F0F] leading-7 flex-1 font-medium">{a.action}</p>
              ...
              {/* Short-term */}
              <SectionCard>
                <SectionTitle icon={Calendar} color="text-[#7C3AED]">{t.shortTermActions}</SectionTitle>
                <div className="space-y-4">
              ...
              {analysisData.winningStrategy.contentSeries && analysisData.winningStrategy.contentSeries.length > 0 && (
                <SectionCard>
                  <SectionTitle icon={Sparkles} color="text-[#7C3AED]">{locale === "ar" ? "سلاسل محتوى مقترحة" : "Recommended Content Series"}</SectionTitle>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {analysisData.winningStrategy.contentSeries.map((s, i) => (
                      <div key={i} className="p-5 rounded-2xl border-2 border-[#7C3AED]/20 bg-[#7C3AED]/5">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-black text-xl text-[#0A1F0F]">{s.name}</h4>
                          <span className="px-3 py-1 rounded-lg bg-[#006C35] text-white text-sm font-bold">{s.platform}</span>
                        </div>
                        <p className="text-lg text-[#2D5A3D] leading-7">{s.description}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <SectionCard>
                  <SectionTitle icon={Lightbulb} color="text-[#7C3AED]">{t.contentGaps}</SectionTitle>
                  <div className="space-y-3">
                    {analysisData.winningStrategy.contentGaps.map((g, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                        <Lightbulb className="h-5 w-5 mt-1 shrink-0 text-[#7C3AED]" />
                        <p className="text-lg text-[#2D5A3D] leading-7">{g}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
                  ...
                  <div className="space-y-3">
                    {analysisData.winningStrategy.differentiators.map((d, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-[#F8FBF8] border border-[#D4EBD9]">
                        ...
                        <p className="text-lg text-[#2D5A3D] leading-7">{d}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
      ...
      {loading && (
        <div className="rounded-2xl border-2 border-[#D4EBD9] bg-gradient-to-br from-[#F0F7F2] via-white to-[#F8FBF8] flex flex-col items-center justify-center py-24 px-8 text-center">
          <div className="relative mb-8">
            ...
            <div className="absolute -top-2 -right-2 w-10 h-10 rounded-full bg-gradient-to-br from-[#7C3AED] to-[#A78BFA] flex items-center justify-center shadow-lg animate-spin" style={{ animationDuration: "3s" }}>
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <p className="text-xl text-[#5A8A6A] font-bold">{t.analyzing}...</p>
          </div>
          <div className="mt-8 w-full max-w-md">
            <div className="h-2 bg-[#E8F0EA] rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#006C35] to-[#7C3AED] rounded-full animate-pulse" style={{ width: "60%", animation: "loading-bar 8s ease-in-out infinite" }} />
            </div>
          </div>
      ...
      {!analysisData && !loading && (
        <div className="rounded-2xl border-2 border-dashed border-[#D4EBD9] bg-white flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[#5A8A6A] text-2xl font-medium">{t.noAnalysisYet}</p>
        </div>
      )}
    </div>
  );
